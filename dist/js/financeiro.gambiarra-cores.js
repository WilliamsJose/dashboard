...
window.addWidgetNew = function() {
   ...
   // gambiarra
   let botoes = [];
   botoes.push(document.querySelector('#changeAdmitidos0'));
   botoes.push(document.querySelector('#changeDemitidos0'));
   $(botoes).click(changeChartColors(botoes, tChart.data.datasets));
   //gambiarra
...
// botoes de cores do chart
function colorButtons(tipo, labels, datasets, i){
   let retorno = '';

   if ( tipo === 'doughnut' || tipo === 'pie' ){
      for(tChartLabel in labels){
         retorno += '<li><label for="change'+ labels[tChartLabel] + i +'" class="btn btn-xs btn-info lb">' + labels[tChartLabel] + '</label></li>' +
         '<input type="color" id="change'+ labels[tChartLabel] + i +'" class="inn" style="display:none">';
      };
   } else {
      for(tChartDataset in datasets){
         retorno += '<li><label for="change'+ datasets[tChartDataset].label + i +'" class="btn btn-xs btn-info lb">' + datasets[tChartDataset].label + '</label></li>' +
         '<input type="color" id="change'+ datasets[tChartDataset].label + i +'" class="inn" style="display:none">';
      };
   };
...
// mudar cor dos labels ou datasets - gambiarra
function changeChartColors(botoes, chart, bot = 0, index = 0){
   for(bot in botoes){
      $(botoes[bot]).on('change', function(){
         for (index in botoes){
            chart[index].backgroundColor = '#fff';
         };
      });
   };
};
